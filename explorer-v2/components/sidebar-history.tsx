...
const SearchItem = ({
  ...
  <SidebarMenuItem>
    <SidebarMenuButton asChild isActive={isActive}>
      <Link href={`/search/${search.id}`} onClick={() => setOpenMobile(false)}>
...
export function SidebarHistory({ user }: { user: User | undefined }) {
  ...
  const {
    ...
    isLoading,
    mutate,
  } = useSWR<Array<Search>>(user ? '/api/history' : null, fetcher, {
    fallbackData: [],
  });
  ...
  const handleDelete = async () => {
    const deletePromise = fetch(`/api/search?id=${deleteId}`, {
      method: 'DELETE',
    });
      ...
      <SidebarGroup>
        <SidebarGroupContent>
          <div className="text-zinc-500 w-full flex flex-row justify-center items-center text-sm gap-2">
            <div>Login to save and revisit previous searches!</div>
          </div>
        </SidebarGroupContent>
      </SidebarGroup>
    );
  ...
  const groupSearchesByDate = (searches: Search[]): GroupedSearches => {
    const now = new Date();
      ...
      <SidebarGroup>
        <SidebarGroupContent>
          <SidebarMenu>
            {history &&
              (() => {
                const groupedSearches = groupSearchesByDate(history);
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete your search and remove it
              from our servers.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
