...
export function Copilot({
  ...
  return (
    <div
      className={`${isExpanded ? "sticky top-0 flex flex-col justify-end h-screen" : ""}`}
    >
      <div
      ...
      >
        <div
          className={`flex flex-col max-w-full my-4 ${isExpanded ? "shadow-lg" : ""}`}
        >
          {isExpanded ? (
            <div className="bg-blue-500 text-white p-2 rounded-t-lg flex items-center">
              <span className="mx-2 uppercase tracking-widest text-xl">
                Copilot
              </span>
              <ModelSelect
                model={model}
          ...
          <div
            className={`bg-white overflow-y-auto max-h-full ${isExpanded ? "" : "hidden"}`}
          >
